...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (action === 'geo') {
    const rows = await sql`
      SELECT COALESCE(country, 'XX') AS country, COUNT(*)::int AS views
      FROM page_views
      WHERE created_at > NOW() - INTERVAL '30 days'
  ...
  if (action === 'live') {
    const rows = await sql`
      SELECT COUNT(*)::int AS n FROM sessions WHERE last_seen > NOW() - INTERVAL '60 seconds'
    `
  ...
  if (action === 'events') {
    const rows = await sql`
      SELECT name, path, meta, created_at FROM events ORDER BY created_at DESC LIMIT 50
    `
