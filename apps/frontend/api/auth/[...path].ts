...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // GET /api/auth/totp-setup?token=...
  if (action === 'totp-setup') {
    if (req.method !== 'GET') return res.status(405).end()
    const setupToken = process.env.setup_token
    if (!setupToken) return res.status(403).json({ error: 'Setup bereits abgeschlossen oder nicht aktiviert' })
    if (req.query.token !== setupToken) return res.status(403).json({ error: 'Ungültiger Setup-Token' })
...
<head>
  ...
  <style>
    body { background:#080808;color:#F5F0E8;font-family:monospace;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:24px;margin:0 }
    img  { border:2px solid #C9A84C;border-radius:8px }
    code { background:#111;padding:12px 20px;border-radius:6px;color:#C9A84C;font-size:1.1em;letter-spacing:0.1em }
...
<body>
  ...
  <p>1. Microsoft Authenticator öffnen<br>2. + → Anderes Konto → QR-Code scannen<br>3. "CandleScope" erscheint in der App</p>
  <p>Secret (falls manuell):</p>
  <code>${secret}</code>
  <p>4. Trage <strong>totp_secret</strong> in Vercel ein: <code style="font-size:0.9em">${secret}</code></p>
  <p class="warn">⚠ Danach setup_token aus Vercel-ENV löschen!</p>
</body>
</html>`)
  }
