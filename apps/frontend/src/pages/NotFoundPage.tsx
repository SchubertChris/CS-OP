/* ============================================================
   ...
   Konzept: Crashender Candlestick Chart — der Markt ist abgestürzt.
   Die 404 ist ein Trading-Terminal das einen kompletten Crash zeigt.
   ============================================================ */
...
/* ─── Candlestick data — starts bullish, crashes hard ──────── */
const CANDLES = [
  ...
  { o: 108, c: 90,  h: 112, l: 88,  bull: false }, // crash starts
  { o: 90,  c: 68,  h: 92,  l: 60,  bull: false },
  { o: 68,  c: 45,  h: 70,  l: 38,  bull: false },
  { o: 45,  c: 22,  h: 48,  l: 15,  bull: false }, // full crash
]
      ...
      {numbers.map(n => (
        <motion.div
          ...
          className={`absolute font-mono text-[10px] tracking-wider select-none ${
            n.red ? 'text-[#FF4444]/40' : 'text-[#C9A84C]/30'
          }`}
          style={{ left: `${n.x}%` }}
    ...
    <div className="min-h-screen bg-[#080808] flex flex-col items-center justify-center relative overflow-hidden px-6">
      ...
      >
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#0d0d0d] border border-[#C9A84C]/15 rounded-t-xl">
          ...
          <span className="font-mono text-[11px] text-[#5a5550] tracking-[0.14em]">
            CS-TERMINAL · MARKET DATA · ERROR
          </span>
          <motion.div
        ...
        <div className="bg-[#060606] border-x border-[#C9A84C]/15 p-4 relative overflow-hidden">
          ...
          {/* Price labels */}
          {[100, 75, 50, 25].map((price, i) => (
            <span key={price}
              className="absolute right-5 font-mono text-[9px] text-[#3a3530]"
          ...
          <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="overflow-visible">
              ...
              return (
                <motion.g
                  ...
                  animate={phase !== 'rising' || i < 9
                    ? {
                        opacity: 1,
                        y: phase === 'dead' && isCrash ? [0, 0, 40] : 0,
                        rotate: phase === 'dead' && isCrash
                          ? [0, 0, (i % 2 === 0 ? 15 : -15)]
                          : 0,
                      }
                    : { opacity: 0, y: 20 }
                  ...
                  transition={{
                    ...
                    y: isCrash && phase === 'dead'
                      ? { delay: 2.8 + (i - 9) * 0.12, duration: 0.6, ease: 'easeIn' }
                      : { delay: i * 0.08, duration: 0.3 },
                    rotate: isCrash && phase === 'dead'
                      ? { delay: 2.8 + (i - 9) * 0.12, duration: 0.5 }
                      : {},
                  }}
                ...
                >
                  ...
                  <line x1={x} y1={top} x2={x} y2={bot}
                    stroke={c.bull ? '#C9A84C' : '#FF4444'}
                    strokeOpacity={c.bull ? 0.6 : 0.8}
            ...
            {(phase === 'crash' || phase === 'dead') && (
              ...
              >
                <motion.path
                  d={`M${PAD.l + spacing * 10},${H - PAD.b - 80 * scale}
                     L${PAD.l + spacing * 11.5},${H - PAD.b - 30 * scale}
                     L${PAD.l + spacing * 10.8},${H - PAD.b - 30 * scale}
                     L${PAD.l + spacing * 11},${H - PAD.b - 10 * scale}
                     L${PAD.l + spacing * 11.4},${H - PAD.b - 10 * scale}
                     L${PAD.l + spacing * 11},${H - PAD.b - 30 * scale}
                     L${PAD.l + spacing * 12},${H - PAD.b - 30 * scale}Z`}
                  fill="#FF4444"
                  fillOpacity="0.25"
                  stroke="#FF4444"
                  strokeWidth="0.5"
          ...
          {phase === 'crash' && (
            ...
            >
              <motion.span
                className="font-display text-5xl font-bold text-[#FF4444] tracking-[0.1em]"
        ...
        <div className="flex items-center gap-4 px-4 py-2 bg-[#0a0a0a] border-x border-b border-[#C9A84C]/15 rounded-b-xl overflow-hidden">
          ...
          <motion.span
            className="font-mono text-[10px] text-[#FF4444] ml-auto"
      ...
      >
        ...
        {/* Red underline */}
        <motion.div
          className="absolute -bottom-2 left-0 right-0 h-0.5 bg-[#FF4444]"
          initial={{ scaleX: 0 }}
          animate={{ scaleX: 1 }}
      ...
      >
        <div className="flex items-center justify-center gap-2 mb-3">
          ...
          <span className="font-mono text-[12px] tracking-[0.16em] text-[#FF4444] uppercase">
            Market Position Not Found
          </span>
        </div>
        <p className="text-[#9A9590] text-base leading-relaxed">
          Diese Seite hat gecrasht wie ein Penny Stock im Bärenmarkt.
          Kein Stop-Loss konnte das verhindern.
        </p>
        <motion.p
          className="font-mono text-[11px] text-[#3a3530] mt-3 tracking-[0.08em]"
      ...
      >
        <motion.span
          className="font-mono text-[10px] text-[#2a2a2a] tracking-[0.14em]"
