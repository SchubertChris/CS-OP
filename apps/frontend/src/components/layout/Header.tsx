...
function DesktopTooltip({ text }: { text: string }) {
  return (
    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 px-3 py-1.5 bg-[var(--cs-s2)] border border-gold/20 text-[var(--cs-text-2)] text-[10px] tracking-[0.1em] uppercase whitespace-nowrap rounded-lg shadow-xl shadow-black/60 pointer-events-none opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-[opacity,transform] duration-200 delay-100 z-50">
      {text}
      <span className="absolute -top-[5px] left-1/2 -translate-x-1/2 w-2 h-2 bg-[var(--cs-s2)] border-l border-t border-gold/20 rotate-45" />
    </div>
  )
}
  ...
  return (
    <>
      ...
      <header className={`
        ...
        ${scrolled
          ? 'h-16 backdrop-blur-2xl border-b border-gold/10 shadow-xl shadow-black/40'
          : 'h-20 bg-transparent'
        }
      `}
      style={scrolled ? { background: 'var(--cs-header-bg)' } : undefined}>
        <Link to="/" aria-label="CandleScope Startseite" className="group flex items-center gap-3 shrink-0">
          ...
          <span className="font-display tracking-[0.15em] text-[var(--cs-text)] uppercase text-base">
            Candle<span className="text-gold">Scope</span>
          </span>
        </Link>

        {/* Pill-Nav */}
        <nav className="flex items-center bg-[var(--cs-s3)] border border-gold/12 rounded-full px-5 py-1.5 gap-1">
        ...
        <div className="flex items-center gap-3 shrink-0">
          {/* Micro-Animated Theme Toggle switcher */}
          <button onClick={toggle} aria-label="Theme wechseln"
            className="w-8 h-8 flex items-center justify-center rounded-full border border-gold/45 text-[var(--cs-text)] transition-colors duration-200 cursor-pointer bg-transparent relative overflow-hidden group">
          ...
          <a
            ...
            className="flex items-center gap-1.5 text-[11px] tracking-[0.12em] uppercase px-4 py-2 rounded-full border border-gold/45 text-[var(--cs-text)] font-semibold transition-colors duration-200 bg-transparent"
          >
      ...
      <div className="fixed right-0 top-[30%] z-40 lg:hidden flex items-center">
        ...
        <button
          ...
          className="w-8 h-16 flex items-center justify-center border border-gold/20 border-r-0 rounded-l-xl text-gold/60 hover:text-gold transition-colors duration-200 shrink-0"
        ...
        {/* Sidebar Panel */}
        <div
          className="flex flex-col gap-1 py-3 backdrop-blur-xl border border-gold/15 rounded-l-2xl overflow-hidden transition-[opacity] duration-300"
          style={{
        ...
        >
          ...
          <div className="h-px bg-gold/20 mb-1 mx-2 shrink-0" />
            ...
            <NavLink key={to} to={to} aria-label={label}
              className={({ isActive }) => `
                ...
                ${isActive
                  ...
                  : 'text-[var(--cs-text-2)] hover:bg-gold/8 hover:text-gold'
                }
              `}>
              {({ isActive }) => (
                <>
                  <span className={`absolute left-0 top-2 bottom-2 w-0.5 rounded-full transition-opacity duration-300 ${isActive ? 'bg-gold opacity-100' : 'opacity-0'}`} />
          ...
          <div className="h-px bg-gold/20 my-1 mx-2 shrink-0" />

          {/* App Login */}
          <a href={`https://app.candlescope.de/login?theme=${theme}`} aria-label="Zum CandleScope Login"
            className="flex items-center gap-3 px-2 h-10 rounded-xl text-gold hover:bg-gold/8 transition-colors duration-200 shrink-0 overflow-hidden">
          ...
          <Link to="/contact" aria-label="Hire me – Kontakt aufnehmen"
            className="flex items-center gap-3 px-2 h-10 rounded-xl text-[var(--cs-text-2)] hover:bg-gold/8 hover:text-gold transition-colors duration-200 shrink-0 overflow-hidden">
          ...
          <button
            data-cal-link="chris-schubert-9newp6"
            aria-label="Termin vereinbaren"
            className="flex items-center gap-3 px-2 h-10 rounded-xl text-[var(--cs-text-2)] hover:bg-gold/8 hover:text-gold transition-colors duration-200 shrink-0 overflow-hidden w-full text-left cursor-pointer"
          >
          ...
          {/* Theme Toggle */}
          <button onClick={toggle} aria-label="Theme wechseln"
            className="flex items-center gap-3 px-2 h-10 rounded-xl text-[var(--cs-text-3)] hover:bg-gold/8 hover:text-gold transition-colors duration-200 cursor-pointer shrink-0 overflow-hidden w-full">
