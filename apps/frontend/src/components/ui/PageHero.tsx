...
function AnimatedTitle({
  line1, line2, accent,
}: { line1: string; line2: string; accent: 'line1' | 'line2' }) {
...
/* ══════════════════════════════════════════════════════════
   ...
   Mobile:  viewBox klein, top-left, max 40vw breit
   Desktop: groß, rechte Seite, volle Höhe
══════════════════════════════════════════════════════════ */
...
function FinanceBg() {
  ...
  return (
    <>
      ...
      <svg
        className="absolute top-16 left-0 pointer-events-none block md:hidden"
        width="100vw"
        height="260"
        viewBox="0 0 720 140"
...
/* ── Dev: Terminal window with typing code ─────────────── */
function DevBg() {
  const lines = [
  ...
  return (
    <>
      {/* Mobile: terminal top area */}
      <div className="absolute top-16 left-4 right-16 pointer-events-none block md:hidden">
        <motion.div
          className="border border-[#C9A84C]/20 rounded-xl overflow-hidden bg-[#0a0a0a]/60 backdrop-blur-sm"
            ...
            {lines.map((line, i) => (
              ...
              >
                ...
                >
                  ...
                  {i === lines.length - 1 && (
                    <motion.span
                      className="inline-block w-1 h-3 bg-[#C9A84C] ml-0.5 align-middle"
      ...
      {/* Desktop: larger terminal right side */}
      <div className="absolute top-20 right-16 xl:right-24 w-[480px] pointer-events-none hidden md:block">
        <motion.div
          className="border border-[#C9A84C]/15 rounded-2xl overflow-hidden bg-[#0a0a0a]/50 backdrop-blur-md shadow-2xl shadow-black/60"
...
/* ── About: Network ─────────────────────────────────────── */
function AboutBg() {
  // Mobile nodes: top-left area (x 0-55%, y 15-65%)
  const mNodes = [
    { cx: 18, cy: 15, r: 2.5, delay: 0.3 },
    { cx: 42, cy: 25, r: 1.8, delay: 0.6 },
    { cx: 65, cy: 18, r: 2, delay: 0.5 },
    { cx: 28, cy: 40, r: 3, delay: 0.9 },
    { cx: 55, cy: 38, r: 1.8, delay: 1.1 },
    { cx: 78, cy: 32, r: 2, delay: 0.8 },
    { cx: 45, cy: 58, r: 2, delay: 0.7 },
    { cx: 70, cy: 55, r: 1.5, delay: 1.3 },
  ]
  const mEdges = [
    { x1: 18, y1: 15, x2: 42, y2: 25, delay: 0.7 },
    { x1: 42, y1: 25, x2: 65, y2: 18, delay: 0.8 },
    { x1: 42, y1: 25, x2: 28, y2: 40, delay: 0.9 },
    { x1: 65, y1: 18, x2: 78, y2: 32, delay: 1.0 },
    { x1: 65, y1: 18, x2: 55, y2: 38, delay: 1.0 },
    { x1: 28, y1: 40, x2: 45, y2: 58, delay: 1.1 },
    { x1: 55, y1: 38, x2: 45, y2: 58, delay: 1.2 },
    { x1: 78, y1: 32, x2: 70, y2: 55, delay: 1.3 },
    { x1: 45, y1: 58, x2: 70, y2: 55, delay: 1.4 },
  ]

  // Desktop nodes: right side
  const dNodes = [
    { cx: 72, cy: 18, r: 2, delay: 0.3 },
    { cx: 85, cy: 35, r: 1.5, delay: 0.6 },
    { cx: 65, cy: 42, r: 2.5, delay: 0.9 },
    { cx: 90, cy: 55, r: 1.5, delay: 1.2 },
    { cx: 75, cy: 62, r: 2, delay: 0.5 },
    { cx: 60, cy: 70, r: 1.5, delay: 1.4 },
    { cx: 88, cy: 72, r: 2, delay: 0.8 },
  ]
  const dEdges = [
    { x1: 72, y1: 18, x2: 85, y2: 35, delay: 0.7 },
    { x1: 85, y1: 35, x2: 65, y2: 42, delay: 0.9 },
    { x1: 85, y1: 35, x2: 90, y2: 55, delay: 1.0 },
    { x1: 65, y1: 42, x2: 75, y2: 62, delay: 1.1 },
    { x1: 90, y1: 55, x2: 75, y2: 62, delay: 1.2 },
    { x1: 75, y1: 62, x2: 60, y2: 70, delay: 1.3 },
    ...
    <>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none hidden md:block"
        ...
        {[{ cx: 60, cy: 30, delay: 1.2 }, { cx: 88, cy: 32, delay: 1.5 }, { cx: 55, cy: 58, delay: 1.8 }, { cx: 92, cy: 60, delay: 2.0 }].map((dot, i) => (
          <motion.circle key={i} cx={`${dot.cx}%`} cy={`${dot.cy}%`} r="0.8%"
            fill="#C9A84C"
...
/* ── Contact: Morse ─────────────────────────────────────── */
function ContactBg() {
  // Mobile morse: smaller, top-left
  const mMorse = [
    { type: 'dot', x: 8, y: 25, delay: 0.3 },
    { type: 'dot', x: 16, y: 25, delay: 0.5 },
    { type: 'dot', x: 24, y: 25, delay: 0.7 },
    { type: 'dot', x: 32, y: 25, delay: 0.9 },
    { type: 'dot', x: 44, y: 25, delay: 1.2 },
    { type: 'dash', x: 55, y: 25, delay: 1.5 },
    { type: 'dot', x: 66, y: 25, delay: 1.7 },
    { type: 'dot', x: 74, y: 25, delay: 1.9 },
    { type: 'dash', x: 10, y: 45, delay: 2.1 },
    { type: 'dot', x: 22, y: 45, delay: 2.3 },
    { type: 'dash', x: 30, y: 45, delay: 2.5 },
    { type: 'dot', x: 42, y: 45, delay: 2.7 },
    { type: 'dot', x: 52, y: 45, delay: 2.9 },
    { type: 'dot', x: 60, y: 45, delay: 3.1 },
  ...
  const dMorse = [
    ...
    { type: 'dot', x: 74, y: 25, delay: 1.2 },
    { type: 'dot', x: 79, y: 25, delay: 1.5 }, { type: 'dash', x: 83, y: 25, delay: 1.7 },
    { type: 'dot', x: 90, y: 25, delay: 1.9 }, { type: 'dot', x: 93, y: 25, delay: 2.1 },
    { type: 'dash', x: 60, y: 35, delay: 2.4 }, { type: 'dot', x: 67, y: 35, delay: 2.6 },
    { type: 'dash', x: 70, y: 35, delay: 2.8 }, { type: 'dot', x: 77, y: 35, delay: 3.0 },
    { type: 'dot', x: 82, y: 35, delay: 3.3 }, { type: 'dot', x: 85, y: 35, delay: 3.5 },
    { type: 'dot', x: 88, y: 35, delay: 3.7 },
  ]

  const renderMorse = (items: typeof mMorse, scale = 1) => items.map((m, i) =>
    m.type === 'dot' ? (
      <motion.circle key={i} cx={`${m.x}%`} cy={`${m.y}%`} r={`${0.8 * scale}%`}
        fill="#C9A84C"
        initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 0.7, scale: 1 }}
        transition={{ delay: m.delay, duration: 0.3, ease: [0.34, 1.56, 0.64, 1] }} />
    ) : (
      <motion.rect key={i}
        x={`${m.x}%`} y={`${m.y - 0.6 * scale}%`}
        width={`${4 * scale}%`} height={`${1.2 * scale}%`}
        rx="0.5%" fill="#C9A84C"
  ...
  return (
    <>
      ...
      <svg className="absolute top-16 left-0 pointer-events-none block md:hidden"
        ...
        viewBox="0 0 100 80" preserveAspectRatio="xMinYMin meet">
