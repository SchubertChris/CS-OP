  ...
  return (
    <div className="min-h-screen bg-[#080808] flex items-center justify-center px-6">
      ...
      <motion.div className="relative w-full max-w-sm"
        ...
        <h1 className="font-display text-2xl text-[#F5F0E8] text-center tracking-[0.06em] mb-2">Admin Panel</h1>
        <p className="text-[#3a3530] text-sm text-center mb-10 tracking-[0.04em]">CandleScope · Chris Schubert</p>
        ...
        <div className="flex flex-col gap-3">
          ...
          {error && <p className="text-[#FF4444] text-[11px] text-center tracking-[0.06em]">{error}</p>}
        </div>

        <div className="grid grid-cols-3 gap-2 mt-8">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => (
            <button key={n} onClick={() => { if (pin.length < 4) setPin(p => p + n) }}
        ...
        <p className="text-[10px] text-[#2a2a2a] text-center mt-6 tracking-[0.08em]">Phase 1 · Bypass aktiv</p>
      </motion.div>
    </div>
  )
}