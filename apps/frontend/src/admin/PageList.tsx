...
export default function PageList() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl">
      ...
      {/* Header */}
      <div className="mb-8">
        <p className="font-mono text-[11px] tracking-[0.2em] text-[var(--cs-text-2)] uppercase mb-3">── SEITEN</p>
        <div className="flex items-end justify-between gap-4">
          <h1 className="font-display text-3xl text-[var(--cs-text)] tracking-tight">Page Manager</h1>
          <button
      ...
      <div className="mb-5 relative">
        <Search size={14} strokeWidth={1.5} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--cs-text-2)]" />
        <input
          type="text"
          placeholder="Seiten suchen..."
          value={search}
          onChange={e => setSearch(e.target.value)}
          className="w-full bg-[var(--cs-s2)] border border-[var(--cs-text)]/8 rounded-xl pl-10 pr-4 py-3 text-[14px] text-[var(--cs-text)] placeholder:text-[var(--cs-text-3)] font-mono focus:outline-none focus:border-[#C9A84C]/30 transition-colors"
        />
      </div>
      ...
      <div className="flex items-center gap-5 mb-5">
        {[
          ...
          <div key={s.label} className="flex items-center gap-2">
            <span className="font-mono text-[11px] tracking-[0.12em] text-[var(--cs-text-3)]">{s.label}</span>
      ...
      <div className="flex flex-col gap-2">
        {filtered.map((page) => {
          ...
          return (
            <div
              key={page.id}
              className="group relative border border-[var(--cs-text)]/8 rounded-xl bg-[var(--cs-s2)] hover:border-[#C9A84C]/25 hover:bg-[#C9A84C]/3 transition-all duration-200"
            >
              <Link to={`/admin/pages/${page.id}`} className="flex items-center gap-4 p-4 pr-28">
                <div className="w-11 h-11 rounded-xl border border-[var(--cs-text)]/10 bg-[var(--cs-s4)] flex items-center justify-center text-[var(--cs-text-2)] group-hover:text-[#C9A84C] group-hover:border-[#C9A84C]/25 transition-all shrink-0">
                  <Icon size={17} strokeWidth={1.5} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[15px] font-medium text-[var(--cs-text)] group-hover:text-[#C9A84C] transition-colors truncate">
                      {page.title}
                    </span>
                    {page.isSystem && (
                  ...
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-[12px] text-[var(--cs-text-2)]">/{page.slug || ''}</span>
                    <span className="font-mono text-[12px] text-[var(--cs-text-3)]">{page.blocks.length} Block{page.blocks.length !== 1 ? 's' : ''}</span>
                    <span className="font-mono text-[12px] text-[var(--cs-text-3)]">{new Date(page.updatedAt).toLocaleDateString('de-DE')}</span>
                  </div>
                </div>
                <ArrowUpRight size={15} strokeWidth={1.5} className="text-[var(--cs-text-3)] group-hover:text-[#C9A84C]/60 transition-colors shrink-0" />
              </Link>
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                    page.nav?.visible
                      ...
                      : 'text-[var(--cs-text-2)] hover:text-[var(--cs-text)] bg-[var(--cs-s4)]'
                  }`}
                >
                ...
                {!page.isSystem && (
                  <button
                    ...
                    className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--cs-text-2)] hover:text-[#FF4444] hover:bg-[#FF4444]/10 bg-[var(--cs-s4)] transition-all"
                  >
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            <p className="font-mono text-[13px] text-[var(--cs-text-3)] tracking-[0.1em]">Keine Seiten gefunden</p>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {confirmDelete && (
          ...
          >
            <motion.div
              className="bg-[var(--cs-s2)] border border-[#FF4444]/25 rounded-2xl p-6 max-w-sm w-full mx-4"
              initial={{ scale: 0.95, y: 8 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 8 }}
            >
              <p className="font-mono text-[12px] tracking-[0.14em] text-[#FF4444] mb-3">LÖSCHEN</p>
              <p className="text-[15px] text-[var(--cs-text)] mb-1">
                Seite &quot;{pages.find(p => p.id === confirmDelete)?.title}&quot; löschen?
              </p>
              <p className="text-[13px] text-[var(--cs-text-2)] mb-5">
